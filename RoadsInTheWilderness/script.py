...
class Map:
            ...
            if len(line) > 0:
                if line[0] == '[':
                    ...
                    for row_terrain in col_as_list:
                        coordinates_dict[(col_num, row_num)] = Coordinate((col_num, row_num), 0, row_terrain, (num_cols, num_rows))
                        row_num += 1
                    col_num += 1
    ...
    def FindBestPathsFromCity(self, city, resources_dict):
        ...
        while(not best_path_found):
            ...
            q_city, best_path_found = WalkOneLevelInGraphSearchRes(q_city, visited_coordinates_dict_from_city, resources_dict, city)
        ...
        if curr_coordinate_as_coordinate not in visited_coordinates_dict_from_city:
            ...
            if not all_res_found_better_path:
                ...
                if curr_coordinate_as_coordinate.city != 0:
                    intersect_needed_resources = [res for res in needed_resources if res in curr_coordinate_as_coordinate.city.resources]
                    for res in intersect_needed_resources:
                        path_to_res = Path(CityResToKey(city, res), WalkTreeUpAndRetCoordinates(curr_coordinate_node), curr_coordinate_path_cost, 0)
                ...
                for next_coordinate in curr_coordinate_as_coordinate.neigh_coordinates:
                    new_node = curr_coordinate_node.AddSon((next_coordinate, curr_coordinate_path_cost + next_coordinate.terrain.GetCost()))
...
for path in final_paths_list:
    if REMOVE_DUPLICATES_AND_REVERSED:
        if str(path.coordinates_only_repr) not in already_printed_dict and str(path.coordinates_only_repr[::-1]) not in already_printed_dict:
